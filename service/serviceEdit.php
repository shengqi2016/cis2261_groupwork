...
<div id="container" style="width: 80%;margin: auto" >
    ...
    <div id="tableOfOrdersNotConfirmed">
        ...
        $query1 = "UPDATE staff_schedule SET item_Status = 'available' WHERE item_Id = '" . $itemId ."';";
        ...
        if ($result1 && $result) {
            ...
            echo "<h3>The service order unconfirmed successfully.</h3>";
            echo "<h3>Please reprocess it in the dashboard.</h3>";

        }else{
            $query = "UPDATE service_order SET status = 'confirmed' WHERE ord_Id = '" . $ordId ."';";
            $result = $mysqli->query($query);

            $query = "UPDATE staff_schedule SET item_Status = 'confirmed' WHERE item_Id = '" . $itemId ."';";

            $result = $mysqli->query($query);
            echo "<h3>The service order unconfirmed failed</h3>";
